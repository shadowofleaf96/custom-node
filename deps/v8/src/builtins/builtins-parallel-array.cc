...
static Tagged<Object> ArrayParallelCommon(BuiltinArguments args, Isolate* isolate, bool is_filter) {
  ...
  for (uint32_t i = 0; i < num_chunks; ++i) {
    ...
    // Wrap chunk in an outer args array, matching ThreadTask's convention:
    // ExecuteTask deserializes args as an array and spreads them as function arguments.
    v8::Local<v8::Array> args_wrapper = v8::Array::New(v8_isolate, 1);
    args_wrapper->Set(context, 0, chunk).FromJust();
    ...
    if (!serializer.WriteValue(context, args_wrapper).FromMaybe(false)) {
      v8_isolate->ThrowException(v8::Exception::Error(
